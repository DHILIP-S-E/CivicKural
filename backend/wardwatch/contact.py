...
def protect(value: str, tenant_id: str, purpose: str = "citizen-notification") -> str | None:
    ...
    response = boto3.client("kms", region_name=settings.aws_region).encrypt(
        ...
        EncryptionContext={"tenant_id": tenant_id, "purpose": purpose},
    )
    return base64.b64encode(response["CiphertextBlob"]).decode("ascii")


def reveal(ciphertext: str, tenant_id: str, purpose: str = "citizen-notification") -> str:
    settings = get_settings()
